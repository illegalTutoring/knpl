...
5. checkpoint 다운로드
    ...
    - (Optional) GPU 메모리가 6GB이상인 경우
        - Nsf Hifigan checkpoint 다운로드
            - [여기](https://github.com/MLo7Ghinsan/MLo7_Diff-SVC_models/releases/download/diff-svc-necessary-checkpoint/nsf_hifigan.zip)에서 다운로드 받기
            - 위에서 압축해제한 폴더로 옮겨서 "여기에 압축해제" 해버리기
...
2. 학습 끝나면 이제 결과물을 뽑을 차례
    ...
    4. 설정이 끝났으면 file_names list안에 이름만 넣어놨던 파일들을 raw 폴더 안으로 옮겨준다.
    5. 그 다음에 다음 코드 실행하면 results 폴더 안에 결과 출력
        ```
        python infer.py
        ```
