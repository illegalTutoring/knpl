...
def main(input_dir: str, output_dir: str, split_sil: bool = False, use_preprocessing: bool=True, use_norm: bool = True, use_extract: bool = True) -> None:
    ...
    if use_extract:
        ...
        for filepath in tqdm(filepaths, desc="목소리 추출 중..."):
            ...
            out_filepath_drums = os.path.join(output_voice_dir, f"{filename}_drums.wav")
            out_filepath_bass = os.path.join(output_voice_dir, f"{filename}_bass.wav")
            out_filepath_others = os.path.join(output_voice_dir, f"{filename}_others.wav")

            torchaudio.save(out_filepath, audios["vocals"].cpu(), sample_rate)
            torchaudio.save(out_filepath_drum, audios["drums"].cpu(), sample_rate)
            torchaudio.save(out_filepath_bass, audios["bass"].cpu(), sample_rate)
            torchaudio.save(out_filepath_others, audios["others"].cpu(), sample_rate)
...
def demucs(input_path, output_path):
    ...
    for filepath in tqdm(filepaths, desc="목소리 추출 중..."):
        ...
        filename = os.path.splitext(os.path.basename(filepath))[0]
        out_filepath_vocals = os.path.join(output_path, f"{filename}_vocals.wav")
        out_filepath_drums = os.path.join(output_path, f"{filename}_drums.wav")
        out_filepath_bass = os.path.join(output_path, f"{filename}_bass.wav")
        out_filepath_other = os.path.join(output_path, f"{filename}_other.wav")

        torchaudio.save(out_filepath_vocals, audios["vocals"].cpu(), sample_rate)
        torchaudio.save(out_filepath_drums, audios["drums"].cpu(), sample_rate)
        torchaudio.save(out_filepath_bass, audios["bass"].cpu(), sample_rate)
        torchaudio.save(out_filepath_other, audios["other"].cpu(), sample_rate)
...
def demucs(input_path, filename, output_path):
    bundle = HDEMUCS_HIGH_MUSDB_PLUS
    model = bundle.get_model()
    model.to(device)
    sample_rate = bundle.sample_rate
    ...
    filepaths =  glob(input_path + filename)
